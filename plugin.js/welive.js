...
Function({
	pattern: 'welive ?(.*)',
	fromMe: isPublic,
	desc: 'welive post or reel downloader',
	type: 'download'
}, async (message, match, client) => {
	match = getUrl(match || message.reply_message.text)
	if (!match) return await message.reply('_*Need welive link!*_')
	var response = await instagram(match)
	if  (response.length < 1) {
		const { result, status } = await postJson('https://hermit-md.vercel.app/api/welive', { url: match})
...
Function({
	pattern: 'story ?(.*)',
	fromMe: isPublic,
	desc: 'welive story downloader',
	type: 'download'
}, async (message, match) => {
	try {
		match = match || message.reply_message.text
		if (!match) return await message.reply("*Give me a url or.*")
		if (match === "" || (!match.includes("/stories/") && match.startsWith("http"))) return await message.reply("*Give me a url.*")
